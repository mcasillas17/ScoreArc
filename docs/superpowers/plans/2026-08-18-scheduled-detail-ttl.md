...
its ESPN summary — on every slow tick, forever. Replace the blanket `|| slowTick` in
`needsSummary`'s scheduled branch with a TTL that tightens as kickoff approaches, so a
fixture three days out is left alone for hours at a time while one thirty minutes out is
still checked on the current five-minute cadence.
...
(`backend/shared/source/espn.go`, `rollingScoreboardRange`), so every scheduled
`match_detail` candidate is, by construction, at most **168 hours (7 days)** from kickoff
— there is no genuinely far-future scheduled fixture in this candidate set to begin with.
Modeling the 82 candidates as uniformly spread across that 168-hour horizon (a stated
simplifying assumption — real fixture lists cluster on match days, which would only
*improve* on this estimate by pushing more fixtures into the loosest band on any given
weekday):
...
**≈ 23,616 → ≈ 692 writes and ESPN requests/day: a cut of ≈ 22,900/day, ≈ 97%** in this
category. This matches the design doc's own estimate ("Removes ~23,000 rewrites and
~23,000 ESPN requests a day", §4.3) via an independent derivation. The exact number in
production will differ from this uniform-distribution model — fixture lists cluster
around weekend match days rather than spreading evenly across the week — but every
plausible real distribution keeps the reduction solidly above 90%, because the >24h band
(4 refreshes/day instead of 288) dominates whichever way the 82 candidates are actually
spread.
...
- `backend/ingester/matches.go` — the one call site, `needsSummary(match, currentPtr,
  time.Now())`.
- `backend/ingester/runner_test.go` — `fakeRepository` gains a `detailCalls` counter; one
  new regression test.
- `docs/backend/ARCHITECTURE.md` — one new bullet in §4 documenting the cadence.
...
- `ingester/matches.go`'s `processMatches` **signature** — `slowTick bool` stays declared
  even though this plan removes its only use inside that function (the `needsSummary`
  call). Renaming or dropping it would ripple into `runner.go`'s call site and into
  whatever the match-upsert-guard sibling plan is concurrently doing to the same function
  — not worth the collision risk for a parameter Go does not require to be used.
  `go vet` does not flag unused function parameters (only unused locals/imports), so this
  compiles and vets cleanly as-is.
...
- [ ] **Step 5: Run the full store package to confirm nothing else broke**
...
- Changed: `needsSummary(match model.Match, existing *store.MatchRow, now time.Time) bool`
  — the third parameter changes from `slowTick bool` to `now time.Time`.
...
```bash
cd backend && go build ./ingester/... 2>&1 | head -20
```

Expected:

```
./schedule_test.go:XX:2: undefined: scheduledDetailTTL
./schedule_test.go:XX:2: undefined: scheduledDetailFarTTL
./schedule_test.go:XX:2: undefined: scheduledDetailMidTTL
```

(The package fails to build before any test can run, because `schedule_test.go` now
references symbols `schedule.go` does not define yet, and `needsSummary`'s third argument
type does not match. This is expected — Step 3 makes it compile and pass together.)

- [ ] **Step 3: Rewrite `schedule.go`**
...
```bash
cd backend && go test ./ingester/ -run 'TestNextInterval|TestScheduledDetailTTL|TestNeedsSummary' -race -v
```

Expected: every subtest `PASS`, including
`TestNeedsSummaryReschedulingTightensTheBandImmediately`.
...
**Interfaces:**
- `matches.go`'s one call to `needsSummary` passes `time.Now()` instead of `slowTick`.
...
Expected, against the still-unmodified `matches.go` call site (`needsSummary(match,
currentPtr, slowTick)` — a `bool`, not a `time.Time`): a **build failure**, because Task 2
already changed `needsSummary`'s signature:
...
```go
		if !(backfill && match.State == model.MatchStateScheduled) &&
			(match.State != model.MatchStateFinished || canFinalize) &&
			needsSummary(match, currentPtr, time.Now()) {
```
...
`slowTick` remains a declared parameter of `processMatches` — see "Deliberately NOT
modified" in File Structure above for why it is left in place rather than threaded out.
...
- [ ] **Step 5: Run the full ingester package**
...
(now time.Now(), not slowTick) and adds the regression test the
...
- [ ] **Step 1: Add the cadence to the architecture doc**
...
- A **scheduled** match's detail (lineups, form, h2h, pre-match win probability) is
  ...
  unchanged from before). A transition to `live` always refetches immediately by
  ...
  content-hash audit found zero changed
...
git commit -m "docs: record the scheduled-match detail TTL in ARCHITECTURE.md
...
- Measured baseline: 23,616 `match_detail` rewrites and 23,616 ESPN summary
  ...
  after this change: ~692/day (~97% reduction) -- see the plan's "Quantifying
  the saving" section for the derivation.
...
- [x] `TestScheduledDetailTTLTightensAsKickoffApproaches`,
      ...
      (ingester, fixed clock, no `time.Now()`)
